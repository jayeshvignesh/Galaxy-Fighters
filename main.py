# Galaxy Fighters - Pygbag Ready Version
...
# Pygbag requires browser-friendly audio formats (.ogg)
BULLET_HIT_SOUND = pygame.mixer.Sound(
    os.path.join("Assets", "Grenade1.ogg")
)

BULLET_FIRE_SOUND = pygame.mixer.Sound(
    os.path.join("Assets", "GunSilencer.ogg")
)

STAGE_SOUND = pygame.mixer.Sound(
    os.path.join("Assets", "bomberman_stage.mp3.ogg")
)

FINAL_SONG = pygame.mixer.Sound(
    os.path.join("Assets", "game_over.ogg")
)
...
def yellow_handle_movement(keys_pressed, yellow):
    ...
    if keys_pressed[pygame.K_s] and yellow.x - VEL > 0:
        yellow.x -= VEL

    if keys_pressed[pygame.K_f] and yellow.x + VEL + yellow.width < BORDER.x:
        yellow.x += VEL

    if keys_pressed[pygame.K_e] and yellow.y - VEL > 0:
        yellow.y -= VEL

    if keys_pressed[pygame.K_d] and yellow.y + VEL + yellow.height + 15 < HEIGHT:
        yellow.y += VEL
...
def draw_final_winner(text, colour):
    ...
    draw_text = FINAL_WINNER_FONT.render(
        text,
        1,
        "white"
    )

    draw_colour = FINAL_WINNER_FONT.render(
        colour,
        1,
        "white"
    )
    ...
    WIN.blit(
        draw_colour,
        (
            WIDTH//2 - draw_colour.get_width()//2,
            HEIGHT//2 - draw_text.get_height()//2 + 200
        )
    )
...
async def draw_stage(number_of_times_run):
    ...
    # Pygbag replacement for pygame.time.delay(3010)
...
async def main(number_of_times_run, red_score, yellow_score):
    ...
    number_of_times_run = await draw_stage(
        number_of_times_run
    )
    ...
    while run:
        ...
        if winner_text != "":
            ...
            # Pygbag replacement for pygame.time.delay(4000)
